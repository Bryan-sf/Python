...
def apagar_produto():
    ...
    while True:
        try:
            resposta_apagar = int(input('Digite o id no qual deseja apagar, ou digite 0 para sair: '))
        except ValueError:
            print('Opcao invalida. Produto nao encontrado.')
            continue
        
        if resposta_apagar == 0:
            break
        
        elif resposta_apagar not in Estoque:
                    print('Produto nao encontrado.')
        else:
            nome_produto = Estoque[resposta_apagar]['nome']
            del Estoque[resposta_apagar]
            print(f'Produto "{nome_produto}" apagado com sucesso.')
            break
...
#momento em que aciono o programa, chamando main para que seja chamado outras funçoes
menu_principal()
